...
def solve(world_map, instructions):
    ...
    world_map = [[l[0],l[1].replace('(', '').replace(')', '').split(', ')] for l in world_map]
    ...
    status, total_steps, current_position = traverse_map(map_dict, instructions, current_position)

    while not status:
        status, total_steps, current_position = traverse_map(map_dict, instructions, current_position, total_steps)
...
def traverse_ghost_map(map_dict, instructions, start_pos):
    ...
    for step in start_pos:
        curr_step = step
        currentTotal = 0
        status, currentTotal, curr_step = iterateInstructions(map_dict, instructions, curr_step, currentTotal)
...
if __name__ == '__main__':
    world_map, instructions = readData('./day8/input.txt')
